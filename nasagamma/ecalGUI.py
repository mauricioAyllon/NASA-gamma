...
Reads a csv file with only one columns titled "counts".
...
if df.shape[1] != 1:
    print("ERROR: data file must have only one column")
else:
    e_units = "channels"
...
ax_reset= plt.axes([0.7, 0.9, 0.1, 0.05])
...
def xSelect(xmin, xmax):
    ...
    #plot_fit(fit)
    fit.plot(plot_type="full", table_scale=[1, 1.8], legend="off",
...
def submit_e(text):
    ...
    mean_vals.sort()
    e_vals.sort()
    pred_erg, efit = pf.ecalibration(mean_vals=mean_vals, erg=e_vals,
                                     channels=x, n=1, e_units=eunits,
                                     plot=True, residual=False, fig=fig,
                                     ax_fit=ax_cal)
    fig.canvas.draw_idle()
