...
class Decay_exp:
    ...
    def fit_single_decay(self):
        ...
        # Perform the fit
        self.fit_result = exp_model.fit(self.y_data, params, t=self.x_data,
                                        weights=1.0 / self.yerr)
    ...
    def fit_double_decay(self):
        ...
        C_guess = self.y_data.min()
        params = exp_model.make_params(A1=A_guess1, A2=A_guess2, k1=k_guess1,
                                         k2=k_guess2, C=C_guess)
        # Perform the fit
        self.fit_result = exp_model.fit(self.y_data, params, t=self.x_data,
                                        weights=1.0 / self.yerr)
    ...
    def plot(self, ax_fit=None, ax_res=None, show_components=False):
        ...
        ax_fit.errorbar(x=self.x_data, y=self.y_data, yerr=self.yerr, fmt="bo", alpha=0.5, capsize=6,
                        markersize=5, label="data")
        if self.degree == 1:
            ax_fit.plot(self.x_data, best_fit, "r", lw=3, alpha=0.7,
                        label=f"Decay constant = {best_values['k']:.3E}")
            if show_components:
                A = best_values["A"]
                k = best_values["k"]
                C = best_values["C"]
                ax_fit.plot([self.x_data.min(), self.x_data.max()], [C, C],
                            lw=2, label="C")
                ax_fit.plot(self.x_data,
                            single_decay_plus_constant(t=self.x_data, 
                                                       A=A, k=k, C=0), label="Exponential")
        elif self.degree == 2:
            ...
            if show_components:
                ...
                ax_fit.plot(self.x_data,
                            single_decay_plus_constant(t=self.x_data, 
                                                       A=A1, k=k1, C=0), label="Exponential 1")
                ax_fit.plot(self.x_data,
                            single_decay_plus_constant(t=self.x_data, 
                                                       A=A2, k=k2, C=0), label="Exponential 2")
        ax_fit.legend()
